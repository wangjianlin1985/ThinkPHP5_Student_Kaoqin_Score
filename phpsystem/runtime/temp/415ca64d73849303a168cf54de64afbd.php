...
<script>
		...
		<div class="col-md-12 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<?php echo url('TermInfo/frontAdd'); ?>" style="display:none;">添加学期信息</a></li>
				</ul>
			  	<!-- Tab panes -->
				    			...
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					...
				    					<?php
				    	            		...
				    	            		/*遍历记录*/
				    					if(is_array($termInfoRs) || $termInfoRs instanceof \think\Collection): $i = 0; $__LIST__ = $termInfoRs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$termInfo): $mod = ($i % 2 );++$i;?>
 										<tr>
 											<td><?php echo $currentIndex++; ?></td>
 											<td><?php echo $termInfo['termId']; ?></td>
 											<td><?php echo $termInfo['termName']; ?></td>
 											<td>
 												<a href="<?php echo url('TermInfo/frontshow',array('termId'=>$termInfo['termId'])); ?>"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="termInfoEdit('<?php echo $termInfo['termId']; ?>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="termInfoDelete('<?php echo $termInfo['termId']; ?>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
	...
	<div style="display:none;">
		...
		<form name="termInfoQueryForm" id="termInfoQueryForm" action="<?php echo url('TermInfo/frontlist'); ?>" class="mar_t15" method="post">
...
function termInfoEdit(termId) {
	$.ajax({
		url :  "<?php echo url('TermInfo/update'); ?>?termId=" + termId ,
		type : "get",
		dataType: "json",
...
function termInfoDelete(termId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url: "<?php echo url('TermInfo/deletes'); ?>",
			data : {
				termIds : termId,
			},
			dataType: "json",
			success : function (obj) {
				if (obj.success) {
					...
					//location.href="<?php echo url('TermInfo/frontlist'); ?>";
				}
				else 
					alert(obj.message);
			},
...
function ajaxTermInfoModify() {
	$.ajax({
		url :  "<?php echo url('TermInfo/update'); ?>",
		type : "post",
		dataType: "json",
