...
<script>
		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<?php echo url('Course/frontAdd'); ?>" style="display:none;">添加课程信息</a></li>
				</ul>
			  	<!-- Tab panes -->
				    			...
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					...
				    					<?php
				    	            		...
				    	            		/*遍历记录*/
				    					if(is_array($courseRs) || $courseRs instanceof \think\Collection): $i = 0; $__LIST__ = $courseRs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$course): $mod = ($i % 2 );++$i;?>
 										<tr>
 											<td><?php echo $currentIndex++; ?></td>
 											<td><?php echo $course['courseNo']; ?></td>
 											<td><?php echo $course['courseName']; ?></td>
 											<td><?php echo $course['teacherName']; ?></td>
 											<td><?php echo $course['coursePlace']; ?></td>
 											<td><?php echo $course['courseCount']; ?></td>
 											<td><?php echo $course['courseScore']; ?></td>
 											<td>
 												<a href="<?php echo url('Course/frontshow',array('courseNo'=>$course['courseNo'])); ?>"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="courseEdit('<?php echo $course['courseNo']; ?>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="courseDelete('<?php echo $course['courseNo']; ?>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="courseQueryForm" id="courseQueryForm" action="<?php echo url('Course/frontlist'); ?>" class="mar_t15" method="post">
			<div class="form-group">
				<label for="courseNo">课程编号:</label>
				<input type="text" id="courseNo" name="courseNo" value="<?php echo $courseNo; ?>" class="form-control" placeholder="请输入课程编号">
			</div>
			<div class="form-group">
				<label for="courseName">课程名称:</label>
				<input type="text" id="courseName" name="courseName" value="<?php echo $courseName; ?>" class="form-control" placeholder="请输入课程名称">
			</div>
			<div class="form-group">
				<label for="teacherName">任课教师:</label>
				<input type="text" id="teacherName" name="teacherName" value="<?php echo $teacherName; ?>" class="form-control" placeholder="请输入任课教师">
			</div>
			<div class="form-group">
				<label for="coursePlace">上课地点:</label>
				<input type="text" id="coursePlace" name="coursePlace" value="<?php echo $coursePlace; ?>" class="form-control" placeholder="请输入上课地点">
			</div>
...
function courseEdit(courseNo) {
	$.ajax({
		url :  "<?php echo url('Course/update'); ?>?courseNo=" + courseNo ,
		type : "get",
		dataType: "json",
...
function courseDelete(courseNo) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url: "<?php echo url('Course/deletes'); ?>",
			data : {
				courseNos : courseNo,
			},
			dataType: "json",
			success : function (obj) {
				if (obj.success) {
					...
					//location.href="<?php echo url('Course/frontlist'); ?>";
				}
				else 
					alert(obj.message);
			},
...
function ajaxCourseModify() {
	$.ajax({
		url :  "<?php echo url('Course/update'); ?>",
		type : "post",
		dataType: "json",
