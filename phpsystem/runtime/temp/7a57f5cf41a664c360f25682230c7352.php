...
<script>
		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	<li><a href="__PUBLIC__/index.php">首页</a></li>
			    	<li role="presentation" class="active"><a href="#attendanceListPanel" aria-controls="attendanceListPanel" role="tab" data-toggle="tab">学生点名列表</a></li>
			    	<li role="presentation" ><a href="<?php echo url('Attendance/frontAdd'); ?>" style="display:none;">添加学生点名</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>记录编号</td><td>学生</td><td>课程</td><td>时间</td><td>状态</td><td>操作</td></tr>
				    					<?php
				    						/*计算起始序号*/
				    	            		$startIndex = ($currentPage-1) * $rows;
				    	            		$currentIndex = $startIndex+1;
				    	            		/*遍历记录*/
				    					if(is_array($attendanceRs) || $attendanceRs instanceof \think\Collection): $i = 0; $__LIST__ = $attendanceRs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$attendance): $mod = ($i % 2 );++$i;?>
 										<tr>
 											<td><?php echo $currentIndex++; ?></td>
 											<td><?php echo $attendance['attendanceId']; ?></td>
 											<td><?php echo $attendance['studentObjF']['studentName']; ?></td>
 											<td><?php echo $attendance['courseObjF']['courseName']; ?></td>
 											<td><?php echo $attendance['timeInfoObjF']['timeInfoName']; ?></td>
 											<td><?php echo $attendance['attendanceStateObjF']['stateName']; ?></td>
 											<td>
 												<a href="<?php echo url('Attendance/frontshow',array('attendanceId'=>$attendance['attendanceId'])); ?>"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="attendanceEdit('<?php echo $attendance['attendanceId']; ?>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="attendanceDelete('<?php echo $attendance['attendanceId']; ?>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="attendanceQueryForm" id="attendanceQueryForm" action="<?php echo url('Attendance/frontlist'); ?>" class="mar_t15" method="post">
            <div class="form-group">
            	<label for="studentObj_studentNumber">学生：</label>
                <select id="studentObj_studentNumber" name="studentObj_studentNumber" class="form-control">
                	<option value="">不限制</option>
	 				<?php
	 				foreach($studentList as $student) {
	 					$selected = "";
 					if($studentObj['studentNumber'] == $student['studentNumber'])
 						$selected = "selected";
	 				?>
            ...
            <div class="form-group">
                ...
                <select id="courseObj_courseNo" name="courseObj_courseNo" class="form-control">
                	<option value="">不限制</option>
	 				<?php
	 				foreach($courseList as $course) {
	 					$selected = "";
 					if($courseObj['courseNo'] == $course['courseNo'])
 						$selected = "selected";
	 				?>
            ...
            <div class="form-group">
            	<label for="timeInfoObj_timeInfoId">时间：</label>
                <select id="timeInfoObj_timeInfoId" name="timeInfoObj_timeInfoId" class="form-control">
                	<option value="0">不限制</option>
	 				<?php
	 				foreach($timeInfoList as $timeInfo) {
	 					$selected = "";
 					if($timeInfoObj['timeInfoId'] == $timeInfo['timeInfoId'])
 						$selected = "selected";
	 				?>
 				 <option value="<?php echo $timeInfo['timeInfoId']; ?>" <?php echo $selected; ?>><?php echo $timeInfo['timeInfoName']; ?></option>
	 				<?php
	 				}
	 				?>
 			</select>
            ...
            <div class="form-group">
            	<label for="attendanceStateObj_stateId">状态：</label>
                <select id="attendanceStateObj_stateId" name="attendanceStateObj_stateId" class="form-control">
                	<option value="">不限制</option>
	 				<?php
	 				foreach($attendanceStateList as $attendanceState) {
	 					$selected = "";
 					if($attendanceStateObj['stateId'] == $attendanceState['stateId'])
 						$selected = "selected";
	 				?>
 				 <option value="<?php echo $attendanceState['stateId']; ?>" <?php echo $selected; ?>><?php echo $attendanceState['stateName']; ?></option>
	 				<?php
	 				}
	 				?>
 			</select>
...
<div id="attendanceEditDialog" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;学生点名信息编辑</h4>
      </div>
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
			 <label for="attendance_attendanceId_edit" class="col-md-3 text-right">记录编号:</label>
			 <div class="col-md-9"> 
			 	<input type="text" id="attendance_attendanceId_edit" name="attendance.attendanceId" class="form-control" placeholder="请输入记录编号" readOnly>
			 </div>
		  </div> 
		  <div class="form-group">
		  	 <label for="attendance_studentObj_studentNumber_edit" class="col-md-3 text-right">学生:</label>
		  	 <div class="col-md-9">
			    <select id="attendance_studentObj_studentNumber_edit" name="attendance_studentObj_studentNumber" class="form-control">
			    </select>
		  	 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="attendance_courseObj_courseNo_edit" class="col-md-3 text-right">课程:</label>
		  	 <div class="col-md-9">
			    <select id="attendance_courseObj_courseNo_edit" name="attendance_courseObj_courseNo" class="form-control">
			    </select>
		  	 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="attendance_timeInfoObj_timeInfoId_edit" class="col-md-3 text-right">时间:</label>
		  	 <div class="col-md-9">
			    <select id="attendance_timeInfoObj_timeInfoId_edit" name="attendance_timeInfoObj_timeInfoId" class="form-control">
			    </select>
		  	 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="attendance_attendanceStateObj_stateId_edit" class="col-md-3 text-right">状态:</label>
		  	 <div class="col-md-9">
			    <select id="attendance_attendanceStateObj_stateId_edit" name="attendance_attendanceStateObj_stateId" class="form-control">
			    </select>
		  	 </div>
		  </div>
		</form> 
...
function attendanceEdit(attendanceId) {
	$.ajax({
		url :  "<?php echo url('Attendance/update'); ?>?attendanceId=" + attendanceId ,
		type : "get",
		dataType: "json",
		success : function (attendance, response, status) {
			if (attendance) {
				...
				$.ajax({
					url: "<?php echo url('Attendance/listAll'); ?>",
					type: "get",
					dataType: "json",
					success: function(students,response,status) { 
						$("#attendance_studentObj_studentNumber_edit").empty();
						var html="";
		        		$(students).each(function(i,student){
		        			html += "<option value='" + student.studentNumber + "'>" + student.studentName + "</option>";
		        		});
		        		$("#attendance_studentObj_studentNumber_edit").html(html);
		        		$("#attendance_studentObj_studentNumber_edit").val(attendance.studentObj);
					}
				});
				$.ajax({
					url: "<?php echo url('Attendance/listAll'); ?>",
					...
					success: function(courses,response,status) { 
		        		...
		        		$("#attendance_courseObj_courseNo_edit").val(attendance.courseObj);
					}
				});
				$.ajax({
					url: "<?php echo url('Attendance/listAll'); ?>",
					type: "get",
					dataType: "json",
					success: function(timeInfos,response,status) { 
						$("#attendance_timeInfoObj_timeInfoId_edit").empty();
						var html="";
		        		$(timeInfos).each(function(i,timeInfo){
		        			html += "<option value='" + timeInfo.timeInfoId + "'>" + timeInfo.timeInfoName + "</option>";
		        		});
		        		$("#attendance_timeInfoObj_timeInfoId_edit").html(html);
		        		$("#attendance_timeInfoObj_timeInfoId_edit").val(attendance.timeInfoObj);
					}
				});
				$.ajax({
					url: "<?php echo url('Attendance/listAll'); ?>",
					...
					success: function(attendanceStates,response,status) { 
						$("#attendance_attendanceStateObj_stateId_edit").empty();
						var html="";
		        		$(attendanceStates).each(function(i,attendanceState){
		        			html += "<option value='" + attendanceState.stateId + "'>" + attendanceState.stateName + "</option>";
		        		});
		        		$("#attendance_attendanceStateObj_stateId_edit").html(html);
		        		$("#attendance_attendanceStateObj_stateId_edit").val(attendance.attendanceStateObj);
					}
				});
...
function attendanceDelete(attendanceId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url: "<?php echo url('Attendance/deletes'); ?>",
			data : {
			...
			success : function (obj) {
				if (obj.success) {
					...
					//location.href="<?php echo url('Attendance/frontlist'); ?>";
				}
				else 
					alert(obj.message);
			},
...
/*ajax方式提交学生点名信息表单给服务器端修改*/
function ajaxAttendanceModify() {
	$.ajax({
		url :  "<?php echo url('Attendance/update'); ?>",
		type : "post",
		dataType: "json",
